...
def get_boundaries_from_ranges(
    variable_format,
    M_range,
    q_range,
    lambdatilde_range=(0.0, 5000.0),
    s1_range=(-0.99, 0.99),
    s2_range=(-0.99, 0.99),
    chi_range=(-0.99, 0.99),
    ...
    iota_range=(0, np.pi),
    ref_phase_range=(-np.pi, np.pi),
    e_range=(0.0, 0.5),
    meanano_range=(0.1, 1.0),
):
    """
    ...
    ######
    if format_info["spin_format"].find("1x") > -1 and s1_range[0] < 0.0:
        s1_range = (0, s1_range[1])
    if format_info["spin_format"] == "fullspins":
        if s1_range[0] < 0:
            s1_range = (0, s1_range[1])
        if s2_range[0] < 0:
            s2_range = (0, s2_range[1])
    ...
    if format_info["mass_format"] == "mcqlambdatilde":
        if format_info["spin_format"] == "nonspinning":
            boundaries = np.array(
                [
                    [M_range[0], q_range[0], lambdatilde_range[0]],
                    [M_range[1], q_range[1], lambdatilde_range[1]],
                ]
            )
        elif format_info["spin_format"] == "chi":
            boundaries = np.array(
                [
                    [M_range[0], q_range[0], lambdatilde_range[0], chi_range[0]],
                    [M_range[1], q_range[1], lambdatilde_range[1], chi_range[1]],
                ]
            )
        elif format_info["spin_format"] == "s1z":
            boundaries = np.array(
                [
                    [M_range[0], q_range[0], lambdatilde_range[0], s1_range[0]],
                    [M_range[1], q_range[1], lambdatilde_range[1], s1_range[1]],
                ]
            )
            ...
            raise RuntimeError(
                f"Spin format '{format_info['spin_format']}' not yet implemented for mcqlambdatilde"
            )

        # setting spin boundaries for non-tidal waveforms
    elif format_info["spin_format"] == "nonspinning":
        boundaries = np.array([[M_range[0], q_range[0]], [M_range[1], q_range[1]]])
    elif format_info["spin_format"] == "chi":
        boundaries = np.array(
            [
                [M_range[0], q_range[0], chi_range[0]],
                [M_range[1], q_range[1], chi_range[1]],
            ]
        )
    elif format_info["spin_format"] == "s1z":
        boundaries = np.array(
            [
                [M_range[0], q_range[0], s1_range[0]],
                [M_range[1], q_range[1], s1_range[1]],
            ]
        )
    elif format_info["spin_format"] == "s1z_s2z":
        boundaries = np.array(
            [
                [M_range[0], q_range[0], s1_range[0], s2_range[0]],
                [M_range[1], q_range[1], s1_range[1], s2_range[1]],
            ]
        )
    elif format_info["spin_format"] == "s1xz":
        boundaries = np.array(
            [
                [M_range[0], q_range[0], s1_range[0], theta_range[0]],
                [M_range[1], q_range[1], s1_range[1], theta_range[1]],
            ]
        )
    elif format_info["spin_format"] == "s1xyz":
        boundaries = np.array(
            [
                [M_range[0], q_range[0], s1_range[0], theta_range[0], phi_range[0]],
                [M_range[1], q_range[1], s1_range[1], theta_range[1], phi_range[1]],
            ]
        )
    elif format_info["spin_format"] == "s1xz_s2z":
        boundaries = np.array(
            [
                [M_range[0], q_range[0], s1_range[0], theta_range[0], s2_range[0]],
                [M_range[1], q_range[1], s1_range[1], theta_range[1], s2_range[1]],
            ]
        )
        ...
        raise RuntimeError(
            "Boundaries current not implemented for the required format of spins {}: apologies :(".format(
                format_info["spin_format"]
            )
        )
    ...
    if format_info["meanano"]:
        boundaries = np.concatenate(
            [boundaries, [[meanano_range[0]], [meanano_range[1]]]], axis=1
        )
    if format_info["iota"]:
        boundaries = np.concatenate(
            [boundaries, [[iota_range[0]], [iota_range[1]]]], axis=1
        )
    if format_info["phi"]:
        boundaries = np.concatenate(
            [boundaries, [[ref_phase_range[0]], [ref_phase_range[1]]]], axis=1
        )
...
def load_PSD(filename, asd=False, ifo="H1", df=None):
    ...
    if filename.endswith("xml") or filename.endswith("xml.gz"):
        PSD_fseries = lal.series.read_psd_xmldoc(
            load_filename(
                filename, verbose=False, contenthandler=lal.series.PSDContentHandler
            )
        )
        try:
            ...
            raise ValueError(
                "The given PSD file doesn't have an entry for the chosen interferometer {}".format(
                    ifo
                )
            )
        f = np.linspace(
            PSD_fseries.f0,
            PSD_fseries.deltaF * PSD_fseries.data.length,
            PSD_fseries.data.length,
        )
...
####################################################################################################################
def ray_compute_injections_match(
    inj_dict,
    bank,
    metric_obj,
    mchirp_window=0.1,
    symphony_match=False,
    max_jobs=8,
    verbose=True,
):
    """
    ...
    for id_, i in enumerate(range(0, inj_dict["theta_inj"].shape[0], n_injs_per_job)):
        inj_dict_ray_list.append(
            _compute_injections_match_ray.remote(
                i,
                i + n_injs_per_job,
                inj_dict,
                bank,
    ...
    for k in inj_dict_ray_list[0].keys():
        if isinstance(inj_dict_ray_list[0][k], np.ndarray):
            inj_dict[k] = np.concatenate(
                [inj_dict_[k] for inj_dict_ in inj_dict_ray_list]
            )
...
@ray.remote
def _compute_injections_match_ray(
    start_id,
    end_id,
    inj_dict,
    bank,
    ...
    return compute_injections_match(
        local_dict, bank, metric_obj, mchirp_window, symphony_match, worker_id, verbose
    )


def compute_injections_match(
    inj_dict,
    bank,
    metric_obj,
    mchirp_window=0.1,
    ...
    chirp_templates = metric_obj.var_handler.get_mchirp(
        templates[:, [0, 1]], "m1m2_nonspinning"
    )
    ...
    if sky_locs is not None:
        ...
        assert (
            sky_locs.shape[-1] == 3
        ), "Each row of sky location must be composed of three angles! {} given".format(
            sky_locs.shape[-1]
        )
        ...
        if symphony_match:
            raise ValueError(
                "The sky localization must be given if the symphony match is used!"
            )
    ...
    for i in tqdm(
        range(injs.shape[0]),
        desc="Generating an injection-template mapping table",
        disable=not verbose,
    ):
        ...
        while True:
            ...
            mcw += 0.05
            if len(ids_) >= min(templates.shape[0], 1500):
                break
    ...
    if worker_id is None:
        desc = "Computing the {} match: loop on the templates".format(
            "symphony" if symphony_match else "std"
        )
    else:
        desc = "Worker {} - Computing the {} match: loop on the templates".format(
            worker_id, "symphony" if symphony_match else "std"
        )
    it_ = (
        tqdm(
            range(templates.shape[0]),
            desc=desc,
            leave=True,
            mininterval=5,
            maxinterval=20,
    ...
    for i in it_:
        ...
        if len(ids_injs) > 0:
            ...
            if symphony_match:
                current_match_injs = metric_obj.WF_symphony_match(
                    s_WFs[ids_injs], *template_WF
                )
            else:
            ...
            ids_to_change = np.where(inj_dict["match"][ids_injs] < current_match_injs)[
                0
            ]
            if len(ids_to_change) > 0:
                inj_dict["match"][ids_injs[ids_to_change]] = current_match_injs[
                    ids_to_change
                ]
...
def initialize_inj_stat_dict(injs, sky_locs=None):
    ...
    if injs.shape[1] != 12:
        raise ValueError(
            "Wrong input size for the injections: each injection must have 12 dimensions but {} given".format(
                injs.shape[1]
            )
        )
...
def compute_injections_metric_match(inj_dict, bank, tiling, verbose=True):
    ...
    template_dist = (
        np.allclose(bank.templates, injs)
        if (bank.templates.shape == injs.shape)
        else False
    )
    N_argpartiton = 20000
    ...
    if verbose:
        inj_iter = tqdm(
            range(injs.shape[0]),
            desc="Evaluating metric match for injections",
            leave=True,
        )
    else:
        inj_iter = range(injs.shape[0])

    if tiling.flow:
        metric_list = tiling.get_metric(injs, flow=True, kdtree=True)
    ...
    for i in inj_iter:
        ...
        if N_argpartiton < bank.templates.shape[0]:
            id_diff_ok = np.argpartition(np.linalg.norm(diff, axis=1), N_argpartiton)[
                :N_argpartiton
            ]
        ...
        if tiling.flow:
            ...
            metric = tiling[inj_dict["id_tile"][i]].metric

        match_i = 1 - np.sum(
            np.multiply(diff[id_diff_ok], np.matmul(diff[id_diff_ok], metric)), axis=-1
        )
...
def plot_match_histogram(
    matches_metric=None, matches=None, mm=None, bank_name=None, save_folder=None
):
    """
    ...
    if bank_name:
        plt.title("Injection recovery for bank {}".format(bank_name), fontsize=fs + 10)
...
def plot_colormap(
    ...
    fig, axes = plt.subplots(
        datapoints.shape[1] - 1, datapoints.shape[1] - 1, figsize=(fsize, fsize)
    )
    if title:
        fig.suptitle(title)
    ...
    for ax_ in combinations(range(datapoints.shape[1]), 2):
        ...
        if values_label:
            cbar.set_label(values_label, rotation=270, labelpad=15)

        if ax_[0] == 0:
            currentAxis.set_ylabel(
                var_handler.labels(variable_format, latex=True)[ax_[1]], fontsize=fs
            )
        else:
            currentAxis.set_yticks([])
        if ax_[1] == datapoints.shape[1] - 1:
            currentAxis.set_xlabel(
                var_handler.labels(variable_format, latex=True)[ax_[0]], fontsize=fs
            )
        else:
...
def plot_tiles_templates(
    ...
    if isinstance(save_folder, str):
        if not save_folder.endswith("/"):
            save_folder = save_folder + "/"
        if savetag:
            savetag = "_{}".format(savetag)

    if isinstance(dist_ellipse, float):  # computing a tile for each template
        if tiling is None:
            raise ValueError(
                "If ellipses are to be plotted, a tiling object is required but None is given"
            )
        dist_template = []
        for t in tiling:
        ...
        ###
    if templates.shape[0] > 500000:
        ids_ = np.random.choice(templates.shape[0], 500000, replace=False)
    else:
    ...
    fig, axes = plt.subplots(
        templates.shape[1] - 1, templates.shape[1] - 1, figsize=(fsize, fsize)
    )
    if title:
        fig.suptitle(title)
    ...
    for ax_ in combinations(range(templates.shape[1]), 2):
        ...
        currentAxis.scatter(
            templates[ids_, ax_[0]],
            templates[ids_, ax_[1]],
            s=size_template,
            marker="o",
            c="b",
            alpha=0.3,
    ...
    if isinstance(save_folder, str):
        plt.savefig(save_folder + "bank{}.png".format(savetag), transparent=False)
            ...
            for t in tiling:
                ...
                currentAxis.add_patch(
                    matplotlib.patches.Rectangle(
                        t[0].mins[ax_], d[ax_[0]], d[ax_[1]], fill=None, alpha=1
                    )
                )

        if isinstance(save_folder, str):
            plt.savefig(save_folder + "tiling{}.png".format(savetag), transparent=False)
    ...
    if isinstance(injections, np.ndarray):
        ...
        if isinstance(save_folder, str):
            plt.savefig(
                save_folder + "injections{}.png".format(savetag), transparent=False
            )
    ...
    if isinstance(dist_ellipse, float):
        ...
        for ax_ in combinations(range(templates.shape[1]), 2):
            ...
            for i, templ in enumerate(templates):
                ...
                currentAxis.add_patch(
                    get_ellipse(metric_projected, templ[ax_], dist_ellipse)
                )
            # if ax_[0]!=0: currentAxis.set_xlim([-10,10]) #DEBUG
            # currentAxis.set_ylim([-10,10]) #DEBUG
        if isinstance(save_folder, str):
            plt.savefig(
                save_folder + "ellipses{}.png".format(savetag), transparent=False
            )

        # Plot an histogram
    fig, axes = plt.subplots(
        1, templates.shape[1], figsize=(4 * templates.shape[1], 5), sharey=True
    )
    if title:
        fig.suptitle(title)
    # plt.suptitle('Histograms for the bank: {} points'.format(templates.shape[0]), fontsize = fs+10)
    hist_kwargs = {
        "bins": min(50, int(len(templates) / 50 + 1)),
        "histtype": "step",
        "color": "orange",
    }
    for i, ax_ in enumerate(axes):
        ...
        if i == 0:
            ax_.set_ylabel("# templates", fontsize=fs)
    ...
    if isinstance(save_folder, str):
        plt.savefig(save_folder + "hist{}.png".format(savetag), transparent=False)
...
def get_projector(*directions):
    ...
    for i, d in enumerate(directions):
        if not np.allclose(np.linalg.norm(d), 1):
            directions[i] = d / np.linalg.norm(d)
    for n1, n2 in combinations(directions, 2):
        assert np.allclose(np.vdot(n1, n2), 0.0), "Directions must be orthogonal"
...
def split_boundaries(boundaries, grid_list, use_plawspace=True):
    ...
    for i in range(D):
        if i == 0:
            ...
            if use_plawspace:
                g_list = plawspace(
                    boundaries[0, i], boundaries[1, i], -8.0 / 3.0, grid_list[i] + 1
                )  # power law spacing
            else:
                g_list = np.linspace(
                    boundaries[0, i], boundaries[1, i], grid_list[i] + 1
                )  # linear spacing
        else:
...
def get_random_sky_loc(N=None, seed=None):
    ...
    latitude = np.arcsin(np.random.uniform(-1.0, 1.0, N))  # FIXME: check this is right!
...
def save_injs(
    ...
    if isinstance(luminosity_distance, (int, float, complex)):
        ...
        ), "Wrong format for luminosity distance. Must be a float or a tuple of float, not {}".format(
            type(luminosity_distance)
        )
    ...
    if sky_locs is None:
        ...
        assert (
            sky_locs.shape[0] >= N
        ), "The number of sky location values should be the same as the number of injections"
    ...
    process = ligolw_process.register_to_xmldoc(
        xmldoc, program="mbank", paramdict={}, comment=""
    )
    ...
    for i, t_inj in enumerate(np.arange(GPS_start, GPS_end, time_step)):
        ...
        row.chi = (
            row.mass1 * row.spin1z + row.mass2 * row.spin2z
        ) / row.mtotal  # is this the actual chi?
        ...
        for site, det in detectors.items():
            tend = tj + lal.TimeDelayFromEarthCenter(
                det.location, row.longitude, row.latitude, tj
            )
            setattr(row, site + "_end_time", tend.gpsSeconds)
            setattr(row, site + "_end_time_ns", tend.gpsNanoSeconds)
            setattr(
                row, "eff_dist_" + site, row.distance
            )  # this is not d_eff, but there is not such a thing as d_eff for precessing searches...
    ...
    print(
        "Saved {} injections to {}".format(i + 1, filename)
    ...
    if filename.endswith("pkl"):
        ...
        for k, v in inj_stat_dict.items():
            if isinstance(v, list):
                inj_stat_dict[k] = np.array(v)
    else:
        raise ValueError(
            "Only json and pickle are valid formats to load an injection stat dictionary"
        )
...
def save_inj_stat_dict(filename, inj_stat_dict):
    ...
    if filename.endswith("pkl"):
        ...
        raise ValueError(
            "Only json and pickle are valid formats to save an injection stat dictionary"
        )
