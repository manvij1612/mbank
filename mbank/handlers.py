...
class variable_handler(object):
    ...
    def __init__(self):
        ...
        for m_, s_, e_, a_ in itertools.product(
            self.m_formats, self.s_formats, self.e_formats, self.angle_formats
        ):
            ...
            base_mass_dim = 3 if m_ == "mcqlambdatilde" else 2

            self.format_D[format_to_add] = (
                base_mass_dim + D_spins[s_] + D_ecc[e_] + D_angles[a_]
            )
            # self.format_D[format_to_add] = 2+ D_spins[s_] + D_ecc[e_] + D_angles[a_] #dimension of the variable format
            self.format_info[format_to_add] = {
                "D": self.format_D[format_to_add],
                "mass_format": m_,
                "spin_format": s_,
                "eccentricity_format": e_,
                "angle_format": a_,
                "e": (e_.find("e") > -1),
                "meanano": (e_.find("meanano") > -1),
                "iota": (a_.find("iota") > -1),
                "phi": (a_.find("phi") > -1),
            }
        ...
        self.format_info["BBH_components"] = {
            "D": 12,
            "mass_format": "BBH_components",
            "spin_format": "BBH_components",
            "eccentricity_format": "BBH_components",
            "angle_format": "BBH_components",
            "e": True,
            "meanano": True,
            "iota": True,
            "phi": True,
        ...
        self.constraints = {
            "M": (0.0, np.inf),
            "logM": (-np.inf, np.inf),
            "q": (1.0 / self.MAX_Q, self.MAX_Q),
            "Mc": (0.0, np.inf),
            "eta": (1.0 / self.MAX_Q, 0.25),
            "mass1": (0, np.inf),
            "mass2": (0, np.inf),
            "chi": (-self.MAX_SPIN, self.MAX_SPIN),
            "s1z": (-self.MAX_SPIN, self.MAX_SPIN),
            "s2z": (-self.MAX_SPIN, self.MAX_SPIN),
            ...
            "s1": (0.0, self.MAX_SPIN),
            "s2": (0.0, self.MAX_SPIN),
            "theta1": (-np.pi, np.pi),
            "phi1": (-np.pi, np.pi),
            "theta2": (-np.pi, np.pi),
            "phi1": (-np.pi, np.pi),
        ...
        self.theta_getter = {
            ...
            "Mc": lambda x: (x[:, 0] + x[:, 1])
            * np.power(
                np.divide(x[:, 1] * x[:, 0], np.square(x[:, 0] + x[:, 1])), 3.0 / 5.0
            ),
            "eta": lambda x: np.divide(x[:, 1] * x[:, 0], np.square(x[:, 0] + x[:, 1])),
            "chi": lambda x: (x[:, 0] * x[:, 4] + x[:, 7] * x[:, 1])
            / (x[:, 0] + x[:, 1]),
            "s1z": lambda x: x[:, 4],
            "s2z": lambda x: x[:, 7],
            "s1": lambda x: np.linalg.norm(x[:, 2:5], axis=1),
            "theta1": lambda x: np.arctan2(
                np.linalg.norm(x[:, [2, 3]], axis=1) * np.sign(x[:, 2]), x[:, 4]
            ),
            "phi1": lambda x: np.arctan(x[:, 3] / (x[:, 2] + 1e-20)),
            "s2": lambda x: np.linalg.norm(x[:, 5:8], axis=1),
            "theta2": lambda x: np.arctan2(
                np.linalg.norm(x[:, [5, 6]], axis=-1) * np.sign(x[:, 5]), x[:, 7]
            ),
            ...
            "iota": lambda x: x[:, 10],
            "phi": lambda x: x[:, 11],
            "lambdatilde": lambda x: np.zeros(
                x.shape[0]
            ),  # coz theta not relevant to tides. Avoids crashing.
        }
    ...
    def is_theta_ok(self, theta, variable_format, raise_error=False):
        ...
        for i, l in enumerate(labels):
            is_ok_l = np.logical_and(
                self.constraints[l][0] < theta[..., i],
                theta[..., i] < self.constraints[l][1],
            )
            is_ok = np.logical_and(is_ok, is_ok_l)
            if not np.all(is_ok_l) and raise_error:
                bad_labels.append(l)

        if raise_error and not np.all(is_ok):
            raise ValueError(
                "The given theta does not have an acceptable value for the quantities: {}".format(
                    *bad_labels
                )
            )
    ...
    def switch_BBH(self, theta, variable_format):
        ...
        if self.format_info[variable_format]["mass_format"] in ["m1m2", "logm1logm2"]:
            ids = np.where(theta[:, 0] < theta[:, 1])[0]
            theta[ids, 0], theta[ids, 1] = (
                theta[ids, 1],
                theta[ids, 0],
            )  # switching masses
        elif self.format_info[variable_format]["mass_format"] in ["Mq", "logMq"]:
            ids = np.where(theta[:, 1] < 1)[0]
            theta[ids, 1] = 1.0 / theta[ids, 1]  # switching masses

        elif self.format_info[variable_format]["mass_format"] == "mcqlambdatilde":
            ids = np.where(theta[:, 1] < 1)[0]
            theta[ids, 1] = 1.0 / theta[ids, 1]
        ...
        if self.format_info[variable_format]["spin_format"] in [
            "nonspinning",
            "chi",
            "s1z",
        ]:
    ...
    def labels(self, variable_format, latex=False):
        ...
        if self.format_info[variable_format]["mass_format"] == "m1m2":
            ...
            if latex:
                labels = [r"$\log_{10}m_1$", r"$\log_{10}m_2$"]
            else:
                labels = ["logm1", "logm2"]
            ...
            if latex:
                labels = [r"$\mathcal{M}_c$", r"$q$", r"$\tilde{\Lambda}$"]
            else:
                labels = ["Mc", "q", "lambdatilde"]
        ...
        if self.format_info[variable_format]["spin_format"] == "nonspinning":
            ...
            if latex:
                labels.extend([r"$s_{1z}$"])
            else:
                labels.extend(["s1z"])
        elif self.format_info[variable_format]["spin_format"] == "s1z_s2z":
            if latex:
                labels.extend([r"$s_{1z}$", r"$s_{2z}$"])
            else:
                labels.extend(["s1z", "s2z"])
        elif self.format_info[variable_format]["spin_format"] == "s1xz":
            if latex:
                labels.extend([r"$s_{1}$", r"$\theta_1$"])
            else:
                labels.extend(["s1", "theta1"])
        elif self.format_info[variable_format]["spin_format"] == "s1xyz":
            if latex:
                labels.extend([r"$s_{1}$", r"$\theta_1$", r"$\phi_1$"])
            else:
                labels.extend(["s1", "theta1", "phi1"])
        elif self.format_info[variable_format]["spin_format"] == "s1xz_s2z":
            if latex:
                labels.extend([r"$s_{1}$", r"$\theta_1$", r"$s_{2z}$"])
            else:
                labels.extend(["s1", "theta1", "s2z"])
        elif self.format_info[variable_format]["spin_format"] == "s1xyz_s2z":
            if latex:
                labels.extend([r"$s_{1}$", r"$\theta_1$", r"$\phi_1$", r"$s_{2z}$"])
            else:
                labels.extend(["s1", "theta1", "phi1", "s2z"])
        elif self.format_info[variable_format]["spin_format"] == "fullspins":
            if latex:
                labels.extend(
                    [
                ...
                labels.extend(["s1", "theta1", "phi1", "s2", "theta2", "phi2"])

        if self.format_info[variable_format]["e"] and latex:
            labels.append(r"$e$")
        if self.format_info[variable_format]["e"] and not latex:
            labels.append("e")

        if self.format_info[variable_format]["meanano"] and latex:
            labels.append(r"$meanano$")
        if self.format_info[variable_format]["meanano"] and not latex:
            labels.append("meanano")

        if self.format_info[variable_format]["iota"] and latex:
            labels.append(r"$\iota$")
        if self.format_info[variable_format]["iota"] and not latex:
            labels.append("iota")

        if self.format_info[variable_format]["phi"] and latex:
            labels.append(r"$\phi$")
        if self.format_info[variable_format]["phi"] and not latex:
            labels.append("phi")
        ...
        **Currently it doesn't support eccentricity**
        ...
        for row in table:
            if isinstance(row, lsctables.SnglInspiral):
                ...
                raise ValueError(
                    "The given table is not recognized! Needed a  SimInspiral or a SnglInspiral table"
                )
            BBH_components.append(
                [
                    row.mass1,
                    row.mass2,
                    row.spin1x,
    ...
    def get_theta(self, BBH_components, variable_format):
        ...
        """
        BBH_components, squeeze = self._check_theta_and_format(
            BBH_components, variable_format
        )

        assert (
            BBH_components.shape[1] == 12
        ), "The number of BBH parameter is not enough. Expected 12 [m1, m2, s1x, s1y, s1z, s2x, s2y, s2z, e, meanano, iota, phi], given {}".format(
            BBH_components.shape[1]
        )
        ...
        if variable_format == "BBH_components":
            ...
            theta = [
                self.theta_getter[l](BBH_components)
                for l in self.labels(variable_format)
            ]
    ...
    def component_masses_to_symmetric_mass_ratio(self, mass_1, mass_2):
        """
    ...
    def lambda_tilde_to_lambda_1_lambda_2(self, lambdatilde, m1, m2):
        """
        ...
        """
        eta = self.component_masses_to_symmetric_mass_ratio(m1, m2)
        q = m1 / m2
        lambda_1 = (
            13
            / 8
    ...
    def get_BBH_components(self, theta, variable_format):
        ...
        assert theta.shape[1] == self.D(
            variable_format
        ), "The number of BBH parameter doesn't fit into the given variable format. Expected {}, given {}".format(
            self.D(variable_format), theta.shape[1]
        )
        ...
        if self.format_info[variable_format]["mass_format"] == "m1m2":
            ...
            m1, m2 = theta[:, 0] * theta[:, 1] / (1 + theta[:, 1]), theta[:, 0] / (
                1 + theta[:, 1]
            )
            m1, m2 = np.maximum(m1, m2), np.minimum(
                m1, m2
            )  # this is to make sure that m1>m2, also if q is less than 1
        elif self.format_info[variable_format]["mass_format"] == "logMq":
            M = 10 ** theta[:, 0]
            ...
            M = theta[:, 0] / np.power(theta[:, 1], 3.0 / 5.0)
            if not np.all(theta[:, 1] <= 0.25):
                raise ValueError(
                    "Values of the symmetric mass ratio should be all <= 0.25. The given array has some entries not satisfying this: {}".format(
                        theta[:, 1]
                    )
                )
        ...
        if self.format_info[variable_format]["mass_format"] == "mcqlambdatilde":
            spin_offset = 3  # Mc, q, lambdatilde
        else:
            spin_offset = 2
        # dealing with spins
        if self.format_info[variable_format]["spin_format"] == "nonspinning":
            ...
            s1z, s2z = theta[:, spin_offset], theta[:, spin_offset]
        elif self.format_info[variable_format]["spin_format"] == "s1z":
            s1z = theta[:, spin_offset]
        elif self.format_info[variable_format]["spin_format"] == "s1z_s2z":
            s1z, s2z = theta[:, spin_offset], theta[:, spin_offset + 1]
        elif self.format_info[variable_format]["spin_format"] == "s1xz":
            s1x, s1z = theta[:, spin_offset] * np.sin(theta[:, spin_offset+1]), theta[:, spin_offset] * np.cos(
                theta[:, spin_offset+1]
            )
        elif self.format_info[variable_format]["spin_format"] == "s1xyz":
            s1x, s1y, s1z = (
                theta[:, spin_offset] * np.sin(theta[:, spin_offset+1]) * np.cos(theta[:, spin_offset+2]),
                theta[:, spin_offset] * np.sin(theta[:, spin_offset+1]) * np.sin(theta[:, spin_offset+2]),
                theta[:, 2] * np.cos(theta[:, 3]),
            )
        elif self.format_info[variable_format]["spin_format"] == "s1xz_s2z":
            s1x, s1z, s2z = (
                theta[:, spin_offset] * np.sin(theta[:, spin_offset+1]),
                theta[:, spin_offset] * np.cos(theta[:, spin_offset+1]),
                theta[:, spin_offset+2],
            )
        elif self.format_info[variable_format]["spin_format"] == "s1xyz_s2z":
            s1x, s1y, s1z, s2z = (
            ...
            s1x, s1y = theta[:, spin_offset] * np.sin(theta[:, spin_offset+1]) * np.cos(theta[:, spin_offset+2]), theta[
                :, spin_offset
            ] * np.sin(theta[:, spin_offset+1]) * np.sin(theta[:, spin_offset+2])
            s1z = theta[:, spin_offset] * np.cos(theta[:, spin_offset+1])
            s2x, s2y = theta[:, spin_offset+3] * np.sin(theta[:, spin_offset+4]) * np.cos(theta[:, spin_offset+5]), theta[
                :, spin_offset+3
            ] * np.sin(theta[:, spin_offset+4]) * np.sin(theta[:, spin_offset+5])
            s2z = theta[:, spin_offset+3] * np.cos(theta[:, spin_offset+4])

            # dealing with angles and eccentricity (tricky!!)
        assign_var = [
            self.format_info[variable_format]["e"],
            self.format_info[variable_format]["meanano"],
            self.format_info[variable_format]["iota"],
            self.format_info[variable_format]["phi"],
        ]
        ...
        BBH_comps = np.stack(
            [m1, m2, s1x, s1y, s1z, s2x, s2y, s2z, e, meanano, iota, phi], axis=1
        )

        if squeeze:
            BBH_comps = np.squeeze(BBH_comps)
    ...
    def get_mchirp(self, theta, variable_format):
        ...
        if self.format_info[variable_format]["mass_format"] == "m1m2":
            mchirp = np.power(theta[:, 0] * theta[:, 1], 3.0 / 5.0) / np.power(
                theta[:, 0] + theta[:, 1], 1.0 / 5.0
            )
        elif self.format_info[variable_format]["mass_format"] == "logm1logm2":
            mchirp = np.power(
                10 ** theta[:, 0] * 10 ** theta[:, 1], 3.0 / 5.0
            ) / np.power(10 ** theta[:, 0] + 10 ** theta[:, 1], 1.0 / 5.0)
        elif self.format_info[variable_format]["mass_format"] == "Mq":
            mchirp = theta[:, 0] * np.power(
                theta[:, 1] / np.square(theta[:, 1] + 1), 3.0 / 5.0
            )
        elif self.format_info[variable_format]["mass_format"] == "logMq":
            mchirp = 10 ** theta[:, 0] * np.power(
                theta[:, 1] / np.square(theta[:, 1] + 1), 3.0 / 5.0
            )
        elif self.format_info[variable_format]["mass_format"] == "mceta":
            mchirp = theta[:, 0]
    ...
    def get_chiP(self, m1, m2, s1x, s1y, s1z, s2x, s2y):
        ...
        assert np.all(
            q >= 1
        ), "m1 should be greater or equal than m2"  # assert q to be greater than 1
        A1 = 2 + 1.5 / q
        A2 = 2 + 1.5 * q
        ...
        if len(ids_) > 0:
            chip[ids_] = np.sqrt(self.MAX_SPIN - s1z[ids_] ** 2)
    ...
    def get_chiP_2D(self, m1, m2, s1x, s1y, s1z, s2x, s2y, s2z, only_spin1=False):
        ...
        m1, m2, s1x, s1y, s1z, s2x, s2y, s2z = np.atleast_1d(
            m1, m2, s1x, s1y, s1z, s2x, s2y, s2z
        )
        ...
        if only_spin1:
            BH1_gtr_BH2 = np.ones(
                S1_perp_norm.shape, dtype=bool
            )  # only the primary BH will have the in-plane spin
        else:
            BH1_gtr_BH2 = S1_perp_norm >= S2_perp_norm

        # computing effective spin parameters
        chi_eff_1, chi_eff_2 = np.zeros(S1_perp.shape), np.zeros(S2_perp.shape)  # (N,2)

        if np.any(BH1_gtr_BH2):
            chi_eff_1[BH1_gtr_BH2, :] = S_perp[BH1_gtr_BH2] / (
                m1[BH1_gtr_BH2] ** 2 + S2_perp_norm[BH1_gtr_BH2]
            )
        if np.any(~BH1_gtr_BH2):
            chi_eff_2[~BH1_gtr_BH2, :] = S_perp[~BH1_gtr_BH2] / (
                m2[~BH1_gtr_BH2] ** 2 + S1_perp_norm[~BH1_gtr_BH2]
            )
        ...
        if len(ids_1) > 0:
            chi_eff_1[ids_1] = (
                chi_eff_1[ids_1].T
                * np.sqrt(self.MAX_SPIN - s1z[ids_1] ** 2)
                / np.linalg.norm(chi_eff_1[ids_1], axis=1)
            ).T
        if len(ids_2) > 0:
            chi_eff_2[ids_2] = (
                chi_eff_2[ids_2].T
                * np.sqrt(self.MAX_SPIN - s2z[ids_2] ** 2)
                / np.linalg.norm(chi_eff_2[ids_2], axis=1)
            ).T
    ...
    def get_chiP_BBH_components(self, BBH_components, chiP_type="chiP"):
        ...
        assert chiP_type in [
            ...
            "chiP_2D",
            "chiP_2D_BH1",
        ], "Wrong format for the chiP_type: it should be one among ['chiP', 'chiP_2D', 'chiP_2D_BH1'], not {}".format(
            chiP_type
        )
        ...
        if chiP_type == "chiP":
            ...
            chiP_BBH_components[:, [3, 5, 6]] = 0.0
        else:
            chiP_1, chiP_2 = self.get_chiP_2D(
                *chiP_BBH_components[:, :8].T, only_spin1=(chiP_type == "chiP_2D_BH1")
            )
            chiP_BBH_components[:, 2:4] = chiP_1
            chiP_BBH_components[:, 5:7] = chiP_2

        if squeeze:
            chiP_BBH_components = chiP_BBH_components[0, :]
...
class tile(tuple):
    ...
    def __new__(cls, rectangle, metric=None):
        ...
        if metric is None:
            rectangle, metric = rectangle
        if isinstance(rectangle, np.ndarray) or isinstance(rectangle, list):
            rectangle = np.asarray(rectangle)
            assert (
                rectangle.ndim == 2 and rectangle.shape[0] == 2
            ), "The given rectangle must be of shape (2,D) but {} was given".format(
                rectangle.shape
            )
            rectangle = scipy.spatial.Rectangle(rectangle[0, :], rectangle[1, :])
        assert isinstance(
            rectangle, scipy.spatial.Rectangle
        ), "The given rectangle must be either a scipy.spatial.Rectangl or a numpy array but a {} was given".format(
            type(rectangle)
        )
        ...
        assert (
            metric.shape == metric_shape
        ), "The metric must have shape {} but shape {} was given".format(
    ...
    @property
    def center(self):
        return (self.rectangle.maxes + self.rectangle.mins) / 2.0
    ...
    def split(self, axis=None, n=2):
        ...
        for i in range(1, n):
            left_rect, right_rect = tile_to_split.rectangle.split(
                axis, self.rectangle.mins[axis] + i / float(n) * len_d
            )
...
class tiling_handler(list, collections.abc.MutableSequence):
    ...
    def __init__(self, ini=None):
        ...
        if isinstance(ini, str):
            ...
            for t in ini:
                assert (
                    len(t) == 2
                ), "The elements in the input list have the wrong length. They should be broadcastable to a tile object, representing a tuple (Rectangle, Metric)"
                self.append(tile(*t))
        else:
    ...
    def _set_boundaries(self):
        ...
        if len(boundaries) > 0:
            ...
            self.boundaries = scipy.spatial.Rectangle(
                np.min(boundaries[:, 0, :], axis=0), np.max(boundaries[:, 1, :], axis=0)
            )
        else:
            self.boundaries = None
        return
    ...
    def is_inside(self, points):
        ...
        if self.boundaries is None:
            raise ValueError(
                "Tiling is empty: unable to check whether points are inside it"
            )
    ...
    def create_tiling_from_list(
        ...
        if not isinstance(boundaries_list, list):
            if isinstance(boundaries_list, np.ndarray):
                if boundaries_list.ndim == 2 and boundaries_list.shape[0] == 2:
                    boundaries_list = [boundaries_list]
                else:
                    raise ValueError(
                        "If `boundaries_list` is an array, must have shape (2,D)"
                    )
            else:
        ...
        for i, b in enumerate(boundaries_list):
            temp_t_obj = (
                tiling_handler()
            )  # This must be emptied at every iteration!! Otherwise, it gives lots of troubles :(
            if use_ray:
                t_ray_list.append(
                    temp_t_obj.create_tiling_ray.remote(
                        temp_t_obj,
                        b,
                        tolerance,
                        metric_func,
                ...
                )  # adding the newly computed templates to the tiling object
    ...
    def create_tiling_ray(
        ...
        return self.create_tiling(
            boundaries, tolerance, metric_func, max_depth, verbose, worker_id
        )
    ...
    def create_tiling(
        ...
        if len(self) == 0:
            start_metric = metric_func((boundaries[1] + boundaries[0]) / 2.0)
            ...
            if np.any(start_rect.min_distance_point(centers) != 0):
                warnings.warn(
                    "The given boundaries are not consistent with the previous tiling. This may not be what you wanted"
                )
        ...
        if start_rect.volume() < 1e-19:
            warnings.warn(
                "The given boundaries are degenerate (i.e. zero volume) and a single tile was generated: this may not be what you expected"
            )
        ...
        if verbose:
            if worker_id is None:
                desc = "Volume covered by the tiling"
            else:
                desc = "Worker {} - Volume covered by tiling".format(worker_id)
            pbar = tqdm(
                total=100,
                desc=desc,
        ...
        while not (tiles_list == tiles_list_old):
            ...
            for t in tiles_list:
                ...
                if t[1]:
                    if verbose:
                        V_covered += t[0].rectangle.volume()
                    continue
                if isinstance(max_depth, (int, float)):
                    if t[2] >= max_depth:
                        continue  # stop splitting
                ...
                extended_list = [
                    (tile(nt[0].rectangle, metric_0), split_tile, t[2] + 1),
    ...
    def compute_volume(self, metric_func=None):
        ...
        if metric_func is None:
            ...
            for rect, _ in tqdm(
                self.__iter__(), desc="Computing the volume of each tile"
            ):
                ranges = [
                    (rect.mins[i], rect.maxes[i]) for i in range(rect.maxes.shape[0])
                ]
                tile_volume, abserr, out_dict = scipy.integrate.nquad(
                    metric_func_, ranges, opts={"limit": 2}, full_output=True
                )
    ...
    def sample_from_flow(self, N_samples):
        ...
        if self.flow is None:
            raise ValueError("You must train the flow before sampling from it!")
    ...
    def sample_from_tiling(
        ...
        if qmc:
            sampler = scipy.stats.qmc.LatinHypercube(d=D, seed=gen)

        tot_vols, vols = self.compute_volume()
        vols = np.array(vols) / tot_vols  # normalizing volumes

        tiles_rand_id = gen.choice(
            len(vols), N_samples, replace=True, p=None if p_equal else vols
        )
        tiles_rand_id, counts = np.unique(tiles_rand_id, return_counts=True)

        if qmc:
            samples = [
                sampler.random(n=c) * (self[t_id][0].maxes - self[t_id][0].mins)
                + self[t_id][0].mins
                for t_id, c in zip(tiles_rand_id, counts)
            ]
        else:
            samples = [
                gen.uniform(self[t_id][0].mins, self[t_id][0].maxes, (c, D))
                for t_id, c in zip(tiles_rand_id, counts)
            ]
        samples = np.concatenate(samples, axis=0, dtype=dtype)

        if tile_id:
            tile_id_vector = np.concatenate(
                [np.full(c, t_id) for t_id, c in zip(tiles_rand_id, counts)]
            )
    ...
    def save(self, filename, flowfilename=None):
        ...
        for t in self.__iter__():
            out_array.append(
                np.concatenate([[t[0].mins], [t[0].maxes], t[1]], axis=0)
            )  # (D+2,D)
        ...
        if isinstance(flowfilename, str):
            if self.flow is None:
                warnings.warn("The flow model is not trained: unable to save it")
            else:
    ...
    def load(self, filename):
        ...
        try:
            ...
            raise FileNotFoundError(
                "Input tiling file {} does not exist".format(filename)
            )
        for in_ in in_array:
    ...
    def get_metric(self, theta, flow=False, kdtree=False):
        ...
        if flow:
            if self.flow is None:
                raise ValueError(
                    "Cannot evaluate the flow PDF, as the flow is not trained. You can do that with `tiling_handler.train_flow`"
                )
    ...
    def get_metric_distance(self, theta1, theta2, flow=False):
        ...
        if flow:
            if self.flow is None:
                raise ValueError(
                    "Cannot compute the distance using the flow integral as the flow is not trained. You can do so by calling `self.train_flow`."
                )

            D = theta1.shape[-1]
            weight_factor = (
                self.integrate_flow(
                    torch.tensor(theta1, dtype=torch.float32),
    ...
    def integrate_flow(self, theta1, theta2, thetac, N_steps=100):
        ...
        """
        if self.flow is None:
            raise ValueError("You should train the flow before computing this integral")

        assert (theta1.ndim >= thetac.ndim) or (
            theta2.ndim >= thetac.ndim
        ), "The center should have a number of dimensions less or equal than the extrema"

        theta1, theta2 = torch.atleast_2d(theta1), torch.atleast_2d(theta2)
        D = theta1.shape[-1]
        steps = theta1 + torch.einsum(
            "ij,k->kij", theta2 - theta1, torch.linspace(0, 1, N_steps)
        )
        ...
        with torch.no_grad():
            ...
            factor = torch.pow(torch.exp(log_pdfs), 2.0 / D)  # (N*N_steps, )
